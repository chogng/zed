...
pub struct LspInlayHintData {
    ...
    hints_at_selections: Vec<HintTarget>,
...
impl LspInlayHintData {
    ...
    pub fn clear_for_buffers(
        ...
        self.remove_pending_text_edits(|hint| buffer_ids.contains(&hint.target.buffer_id));
...
impl Editor {
    ...
    pub fn accept_inlay_hint(
        ...
        self.refresh_inlay_hints_at_selections(&display_snapshot, cx);
        let Some(inlay_hints) = &self.inlay_hints else {
            return;
        };
        let hints = inlay_hints.hints_at_selections.clone();
        self.apply_inlay_hint_text_edits(hints, None, window, cx);
    }

    pub(crate) fn can_accept_inlay_hint(&self, cx: &App) -> bool {
        self.lsp_data_enabled()
            && !self.read_only(cx)
            && self.inlay_hints.as_ref().is_some_and(|inlay_hints| {
                inlay_hints
                    .hints_at_selections
                    .iter()
                    .any(|target| self.is_buffer_writable(target.buffer_id, cx))
            })
    }

    fn is_buffer_writable(&self, buffer_id: BufferId, cx: &App) -> bool {
        self.buffer
            .read(cx)
            .buffer(buffer_id)
    ...
    pub(crate) fn apply_inlay_hint_text_edits(
        &mut self,
        hints: impl IntoIterator<Item = HintTarget>,
        clicked_label_offset: Option<usize>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> bool {
        ...
        for target in hints {
            ...
            if !self.is_buffer_writable(buffer_id, cx) {
                continue;
            }
            ...
            let already_pending = self
                ...
                .is_some_and(|pending| {
                    pending
                        .hints
                        .iter()
                        .any(|hint| hint.target.hint_id == hint_id)
                });
            if !already_pending {
    ...
    fn apply_pending_inlay_hint_text_edits(
        ...
        } in pending.hints
        {
            if !inlay_hints.added_hints.contains_key(&hint_id)
                || !position.is_valid(&multi_buffer_snapshot)
            {
                continue;
            }
            ...
            if clicked_label_offset
                .is_some_and(|offset| label_part_has_command(cached_hint, offset))
            {
                continue;
            }
    ...
    pub(crate) fn refresh_inlay_hints_at_selections(
        ...
        for selection in self.selections.all_adjusted(display_snapshot) {
            ...
            let first_inlay =
                inlays.partition_point(|inlay| inlay.position.cmp(&start, buffer_snapshot).is_lt());
            for inlay in &inlays[first_inlay..] {
                if inlay.position.cmp(&end, buffer_snapshot).is_gt() {
                    break;
                }
                if !matches!(inlay.id, InlayId::Hint(_))
                    || !inlay.position.is_valid(buffer_snapshot)
                {
                    continue;
                }
                if let Some(buffer_id) = inlay
                    .position
                    .raw_text_anchor()
                    .map(|anchor| anchor.buffer_id)
                {
                    inlay_hints.hints_at_selections.push(HintTarget {
                        buffer_id,
                        hint_id: inlay.id,
    ...
    async fn test_accept_inlay_hints_across_buffers(cx: &mut gpui::TestAppContext) {
        ...
        *resolve_gate.lock() = Some(gate);
        ...
        assert_eq!(
            buffer_texts(cx),
            (
                original_texts.0.clone(),
                "fn second() { let b: u64 = 2; }\n".to_string()
            ),
            "a queued hint hidden by excerpt replacement is skipped"
        );
        editor
            .update(cx, |editor, window, cx| {
                assert_eq!(
                ...
                assert!(
                    !editor.can_accept_inlay_hint(cx),
                    "a hint hidden by excerpt removal is not acceptable"
                );
                editor.accept_inlay_hint(&AcceptInlayHint, window, cx);
            })
            .unwrap();
